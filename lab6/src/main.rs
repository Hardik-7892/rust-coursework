...
fn main() {
    ...
    let handle = thread::spawn(move || 
    {
        ...
        while let Some(c) = iter.next() {
            ...
            match c {
                '\u{1F600}' => {
                    let msg = UpdateMsg::IncrementCell {i};
                    tx.send(msg).unwrap();
                }
                '\u{1F641}' => {
                    let msg = UpdateMsg::DecrementCell {i};
                    tx.send(msg).unwrap();
                }
                '\u{27A1}' => {
    ...
    for msg in rx {
        match msg {
            UpdateMsg::IncrementCell {i} => {v[i] += 1;}
            UpdateMsg::DecrementCell {i} => {v[i] -= 1;}
            UpdateMsg::NewCell => { v.push(0); }
            UpdateMsg::Error => {error = true; }
        }
    }
    handle.join().unwrap();
